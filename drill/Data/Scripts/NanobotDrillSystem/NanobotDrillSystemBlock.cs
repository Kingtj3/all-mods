namespace SpaceEquipmentLtd.NanobotDrillSystem
{
   ...
   public class NanobotDrillSystemBlock : MyGameLogicComponent
   {
      ...
      public const int DRILL_RANGE_DEFAULT_IN_M = 200;
      ...
      private float ComputeRequiredElectricPower()
      {
         ...
         if (_Drill.Enabled)
         {
            if (MyAPIGateway.Session.IsServer && Mod.Log.ShouldLog(Logging.Level.Info)) Mod.Log.Write(Logging.Level.Info, "DrillSystemBlock {0}: ComputeRequiredElectricPower Enabled", Logging.BlockName(_Drill, Logging.BlockNameOptions.None));
         ...
         if (MyAPIGateway.Session.IsServer && Mod.Log.ShouldLog(Logging.Level.Info)) Mod.Log.Write(Logging.Level.Info, "DrillSystemBlock {0}: ComputeRequiredElectricPower {1}", Logging.BlockName(_Drill, Logging.BlockNameOptions.None), required);
         return required;
      }
      ...
      private bool HasRequiredElectricPower(bool drill, bool fill, bool transport)
      {
         ...
         var resourceSink = _Drill.Components.Get<Sandbox.Game.EntityComponents.MyResourceSinkComponent>();
         if (resourceSink != null)
         {
            enought = resourceSink.SuppliedRatioByType(ElectricityId) == 1;
         }
         if (Mod.Log.ShouldLog(Logging.Level.Info)) Mod.Log.Write(Logging.Level.Info, "DrillSystemBlock {0}: HasRequiredElectricPower {1} / {2}", Logging.BlockName(_Drill, Logging.BlockNameOptions.None), enought, required);
